...
public class HomeController {
	...
	public @ResponseBody List<MalwareIncident> getMalwareIncidents() {
		logger.info("Start getMalwareIncident");
		 QueryBuilder builder =QueryBuilders.termQuery("srcCountry","China");
	...
	public @ResponseBody MalwareIncident getMalwareIncident() {
		logger.info("Start getMalwareIncident");
		 QueryBuilder builder =QueryBuilders.termQuery( "_id","AVGK_ntb0soIwxvleMmk");
		
		    SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(builder).build();
		    List<MalwareIncident> incident = elasticsearchTemplate.queryForList(searchQuery, MalwareIncident.class);
